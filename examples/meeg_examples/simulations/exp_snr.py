...
def run_snr_experiment():
    ...
    for snr_log in snr_logs:
        ...
        for iteration in range(n_mc_iterations):
            ...
            # Находим компоненту с лучшей абсолютной корреляцией паттерна
            best_pattern_corr = 0
            best_power_corr = 0
    ...
    ax1.errorbar(snr_logs, mean_pattern_corr, yerr=std_pattern_corr, fmt='-o', capsize=5, color='b')
    ...
    ax2.errorbar(snr_logs, mean_power_corr, yerr=std_power_corr, fmt='-o', capsize=5, color='r')
